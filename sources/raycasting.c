...
t_cords	x_y_intercept(int flag, t_cub3d *data)
{
	t_cords	result;
	...
	if (flag == 1)
	{
		...
		result.y += isRayFacingDown(data->myray.ray_angle) ? TILE_SIZE : 0;
		result.x = data->myplayer.x + (result.y - data->myplayer.y) / tan(data->myray.ray_angle);
	}
	else if (flag == 2)
	{
		result.x = floor(data->myplayer.x / TILE_SIZE) * TILE_SIZE;
		result.x += isRayFacingRight(data->myray.ray_angle) ? TILE_SIZE : 0;
		result.y = data->myplayer.y + (result.x - data->myplayer.x) * tan(data->myray.ray_angle);
	}
	return (result);
}

t_cords	x_y_steps(int flag, t_cub3d *data)
{
	double	xstep;
	double	ystep;
	...
	{
		ystep = TILE_SIZE;
		ystep *= isRayFacingUp(data->myray.ray_angle) ? -1 : 1;

		xstep = TILE_SIZE / tan(data->myray.ray_angle);
		xstep *= (isRayFacingLeft(data->myray.ray_angle) && xstep > 0) ? -1 : 1;
		xstep *= (isRayFacingRight(data->myray.ray_angle) && xstep < 0) ? -1 : 1;
	}
	else if (flag == 2)
	{
		xstep = TILE_SIZE;
		xstep *= isRayFacingLeft(data->myray.ray_angle) ? -1 : 1;

		ystep = TILE_SIZE * tan(data->myray.ray_angle);
		ystep *= (isRayFacingUp(data->myray.ray_angle) && ystep > 0) ? -1 : 1;
		ystep *= (isRayFacingDown(data->myray.ray_angle) && ystep < 0) ? -1 : 1;
	}
	return ((t_cords){xstep, ystep});
}

t_cords	horizontal_increment(double nextHorzTouchX, double nextHorzTouchY,t_cub3d *data,
		int *foundHorzWallHit, t_cords result)
{
	t_cords	wall_hit;
	while (isInsideMap(nextHorzTouchX, nextHorzTouchY))
	{
		double xToCheck = nextHorzTouchX;
		double yToCheck = nextHorzTouchY + (isRayFacingUp(data->myray.ray_angle) ? -EPSILON : 0);
...
t_cords	vertical_increment(double nextVertTouchX, double nextVertTouchY,t_cub3d *data,
		int *foundVertWallHit, t_cords result)
{
	t_cords	wall_hit;
	
	while (isInsideMap(nextVertTouchX, nextVertTouchY))
	{
		double xToCheck = nextVertTouchX + (isRayFacingLeft(data->myray.ray_angle) ? -EPSILON : 0);
...
void	calcul_distance(t_cords vert, t_cords horz, t_cords found_wall_hit, t_cub3d *data)
{
	...
	if (vertHitDistance < horzHitDistance) {
		...
		data->myray.wall_hit_x = vert.x;
		data->myray.wall_hit_y = vert.y;
		data->myray.was_hit_vertical = 1;
	} else {
		data->myray.distance = horzHitDistance;
		data->myray.wall_hit_x = horz.x;
		data->myray.wall_hit_y = horz.y;
...
void	castRay(double rayAngle, t_cub3d *data)
{
	...
	horz_data = x_y_intercept(1, data);
	steps = x_y_steps(1, data);
	horz_wall_hit = horizontal_increment(horz_data.x, horz_data.y, data, &foundHorzWallHit, steps);
	
	// VERTICAL
	vert_data =  x_y_intercept(2, data);
	steps = x_y_steps(2, data);
	vert_wall_hit = vertical_increment(vert_data.x, vert_data.y, data, &foundVertWallHit, steps);
